...
import { deleteCosmetics, fetchCosmetics } from './store/Cosmetics.action'
...
const CosmeticsList = () => {
    ...
    const { entities } = useSelector((state) => state.Cosmetics)
    const loading = useSelector((state) => state.Cosmetics.loading)

    const handleDelete = (id) => {
        dispatch(deleteCosmetics({ id }))
    }

    const handleEdit = (id) => {
        navigate(`/Cosmetics/edit/${id}`)
    }

    const handleAdd = () => {
        navigate(`/Cosmetics/add`)
    }
